...
<!--
  ...
  120-item control set).
  If a later eval wave changes the table, refresh THIS BLOCK ONLY and nothing else moves.
-->

**Errata leads overall — 60.0 vs 47.5 (full-context) and 45.8 (naive top-k RAG) — at 1/43rd the
...
|---|---|---|---|---|---|---|---|---|---|
| **Errata** | **60.0 ± 0.0** | 44.7 ± 0.0 | **67.7 ± 0.0** | **51.5 ± 0.0** | **94.4 ± 0.0** | 0.49 / 0.93 | **2,532** | **$0.0000** | **0.26 / 1.28** |
...
the judge. Counting all 450 rows including abstention, the same runs read **Errata 66.7, naive 56.2,
full-context 54.0**. Errata's `$/Q` is $0.0000257, not zero — the column rounds to four decimals —
and its `$/Q` and latency are measured on `rerunF-wave`, the last cold-cache run; `rerunJ-arith`
changed three answers and replayed the other 447 from cache, which would flatter both columns.
...
answer-for-answer**; the account is in `eval/RESULTS.md`. Across the two published waves,
knowledge-update regressed 100.0 → 94.4 and context cost rose 21% (2,095 → 2,532 tokens/question):
three questions regressed against eleven improved, each one traced, printed rather than dropped.
...
saying otherwise would be false. A sensitivity sweep ships instead — overall is flat at 66.7 across
