...
describe('key builders ', () => {
  ...
  it('emits the documented natural-key shapes (sessions keyed by ordinal, not session_id)', () => {
    expect(keys.session('h1', 5)).toBe('h:h1|s:5');
  ...
  it('two sessions sharing a session_id do not collide (they key on ordinal)', () => {
    expect(keys.session('h1', 17)).not.toBe(keys.session('h1', 47));
    expect(keys.turn('h1', 17, 0)).not.toBe(keys.turn('h1', 47, 0));
  });
