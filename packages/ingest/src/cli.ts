...
async function main(): Promise<void> {
  ...
  const extractor: Extractor =
    extractorName === 'replay' ? new ReplayExtractor(arg('replay-dir', 'fixtures/replay')) : new RuleExtractor();

  const client = new GraphClient({ url, token });
  try {
    await client.verify();
    const t0 = Date.now();
    const s = await ingestHistory(client, history, { extractor, lexiconDir });
