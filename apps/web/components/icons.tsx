/**
 * Vendored Phosphor icons (phosphoricons.com, MIT) — path data copied verbatim from
 * @phosphor-icons/core@2.1.1 "regular", rendered inline so the no-icon-library dependency rule
 * holds the same way the fonts hold the no-external-font rule. currentColor throughout.
 */
import type { SVGProps } from 'react';

function icon(path: string) {
  return function Icon({ className, ...rest }: SVGProps<SVGSVGElement>) {
    return (
      <svg
        viewBox="0 0 256 256"
        width="1em"
        ...
        fill="currentColor"
        aria-hidden="true"
        className={className ? `pico ${className}` : 'pico'}
        {...rest}
      >
        <path d={path} />
      </svg>
...
export const IconCaretLeft = icon(
